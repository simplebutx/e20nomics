...
public class TodayNewsService {
    ...
    public List<PublicTodayNewsResponse> getPublicTodayNews() {
        List<TodayNews> lists = todayNewsRepository.findAllByIsPublishedTrue();
        return lists.stream().map((list) -> new PublicTodayNewsResponse(list.getId(), list.getSummaryTitle(), list.getSummaryText(), list.getCreatedAt()))
                .toList();
    }
    ...
    // 관리자: 오늘의 뉴스 생성하기
    @Transactional
    public TodayNewsGenerateResponse summarize(String text) {
        if (!StringUtils.hasText(text)) {    // null, 공백, 길이를 모두 한번에 체크
            return new TodayNewsGenerateResponse("", "요약할 텍스트가 비어있습니다.", false);
        }
        if (!isEconomicNews(text)) {
            return new TodayNewsGenerateResponse("", "경제 뉴스가 아니므로 요약할 수 없습니다.", false);
        }
        ...
        try {
            ...
            return new TodayNewsGenerateResponse(summaryTitle, summaryText, true);

        } catch (Exception e) {
            e.printStackTrace();
            return new TodayNewsGenerateResponse("", "제목/요약에 실패했습니다.", false);
        }
    }
    ...
    // 관리자: 오늘의 뉴스 저장하기
    @Transactional
    public void saveTodayNews(TodayNewsCreateRequest dto) {
    ...
    public List<TodayNewsResponse> getTodayNewsList() {
        ...
        return lists.stream().map((list) -> new TodayNewsResponse(list.getId(), list.getSummaryTitle(), list.getSummaryText(), list.getIsPublished(), list.getCreatedAt()))
                .toList();
    }
